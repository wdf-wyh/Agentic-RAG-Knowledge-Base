...
4. 仅当【历史对话】中确有先前轮次，且问题明确指代先前内容（如"刚才"、"之前"、"上一个"）→ conversation；若历史为空或写着「无历史对话」，禁止选择 conversation
...
11. 无意义乱码、无法理解的输入 → direct_answer（不要选 conversation）

【重要提醒】
- 你只需要分析问题，不需要回答问题
- 请严格输出 JSON 格式，不要有其他内容
- confidence 应该反映你对判断的确信程度
- conversation 意图要求历史对话非空；首轮对话永远不要选 conversation
